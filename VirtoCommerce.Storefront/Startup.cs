...
namespace VirtoCommerce.Storefront
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            var section = Configuration.GetSection("VirtoCommerce:Endpoint");
            ...
            services.AddAuthorization(options =>
            {
                ...
                options.AddPolicy(AnonymousUserForStoreAuthorizationRequirement.PolicyName,
                                policy => { policy.Requirements.Add(new AnonymousUserForStoreAuthorizationRequirement());
                                    policy.AuthenticationSchemes.Add(SwitchEduDefaults.AuthenticationScheme);
                                });
            });
            ...
            var auth = services.AddAuthentication(options =>
            {
                ...
                options.DefaultChallengeScheme = SwitchEduDefaults.AuthenticationScheme;
            });

            var switchEduSection = Configuration.GetSection("Authentication:SwitchEdu");
            services.AddScoped<CustomSwitchEduAuthenticationEvents>();
            if (switchEduSection.GetChildren().Any())
            {
                auth.AddCookie(CookieAuthenticationDefaults.AuthenticationScheme)
                    .AddSwitchEdu(options =>
                    {
                        options.AddSwitchEduOptions();
                        switchEduSection.Bind(options);
                        options.EventsType = typeof(CustomSwitchEduAuthenticationEvents);
                    }
                );
            }
            ...
            services.AddCors(options =>
            {
                options.AddPolicy(name: MyAllowSpecificOrigins,
                                  policy =>
                                  {
                                      policy.WithOrigins("https://dc57-188-247-194-47.eu.ngrok.io")
                                      .AllowAnyOrigin()
                                      .AllowAnyHeader()
                                      .AllowAnyMethod();
                                  });
